...
export default function PublicRosterPage() {
  ...
  if (!isMounted || !currentDate) return <div className="p-8 text-center text-gray-500">載入中...</div>;
  ...
  const renderTable = (title: string, staffForEntity: Staff[], colorClass: string) => {
    ...
    return (
      <div className="mb-6 md:mb-8 overflow-hidden rounded-lg shadow-sm border border-slate-200">
        <h3 className={`font-bold text-sm md:text-md p-2 md:p-3 border-b bg-white border-l-4 ${colorClass}`}>{title}</h3>
        <div className="overflow-x-auto -mx-2 md:mx-0">
          <table className="w-full border-collapse bg-white text-xs md:text-sm">
            <thead>
              <tr>
                <th className="p-1.5 md:p-2 border bg-slate-50 sticky left-0 z-30 min-w-[70px] md:min-w-[100px] text-left text-slate-500 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">員工</th>
                {days.map(d => {
                  ...
                  return (
                    <th key={d.dateStr} className={`p-0.5 md:p-1 border text-center min-w-[28px] md:min-w-[35px] ${headerBg} ${textColor} ${isToday ? 'border-b-2 md:border-b-4 border-yellow-400' : ''}`}>
                      <div className="text-[10px] md:text-xs font-bold">{d.dateObj.getDate()}</div>
                ...
                <tr key={staff.id}>
                  ...
                  {days.map(d => {
                    ...
                    if (data.day_type === 'rest') {
                      ...
                      badge = <span className="block text-[7px] md:text-[8px] text-emerald-600 font-bold mb-0.5">休</span>;
                    } else if (data.day_type === 'regular') {
                      cellBg = 'bg-red-50 bg-[linear-gradient(45deg,transparent_25%,rgba(255,0,0,0.05)_50%,transparent_75%,transparent_100%)] bg-[length:8px_8px] md:bg-[length:10px_10px]';
                      badge = <span className="block text-[7px] md:text-[8px] text-red-500 font-bold mb-0.5">例</span>;
                    }

                    return (
                      <td key={d.dateStr} className={`border p-0.5 text-center align-top h-10 md:h-12 min-w-[28px] md:min-w-[35px] ${cellBg} ${isToday ? 'border-x-2 border-yellow-300' : ''}`}>
                        {badge}
                          ...
                          {(['M','A','N'] as Shift[]).map(s => {
                            ...
                            return (
                              <div 
                                ...
                                title={`${s === 'M' ? '早' : s === 'A' ? '午' : '晚'}班 ${timeDisplay}`}
                              />
                            );
                          })}
                        </div>
