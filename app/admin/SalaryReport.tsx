        ...
        try {
            ...
            if (settingsResult.data) {
                ...
                if (titlesSetting && titlesSetting.value) {
                    try { jobTitles = JSON.parse(titlesSetting.value); } catch(e) {}
                }
            }

            if (staffResult.data) {
                const getRoleWeight = (roleName: string) => {
                    const index = jobTitles.findIndex((j: any) => (typeof j === 'string' ? j : j.name) === roleName);
                ...
                setStaffList(sorted.map((s: any) => ({ id: s.id, name: s.name, role: s.role })));
            }
        } catch (error) {
            console.error(error);
        }
