...
type Props = {
  ...
  modifyAdjustment: (...args: any[]) => void;
  staffList: any[];
                  ...
                  <td className="p-4 pl-6 align-top min-w-[200px]">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {!isLocked && onOpenSettings && staffId && (
                        <button
                          onClick={(e) => { e.stopPropagation(); onOpenSettings(staffId); }}
                  ...
                  <td className="p-4 align-top space-y-1.5 min-w-[180px]">
                    ...
                    <div className="text-xs text-slate-500 space-y-0.5">
                      <div className="flex justify-between w-40">
                        <span>本薪:</span> <span>${rpt.base_pay.toLocaleString()}</span>
                      </div>
                      <div className="flex justify-between w-40">
                        <span>加班:</span> <span>${(rpt.ot_pay + rpt.holiday_pay).toLocaleString()}</span>
                      </div>
                      ...
                      <div className="flex justify-between w-40">
                        <span>固定津貼:</span> <span>${rpt.fixed_bonus_pay.toLocaleString()}</span>
                      </div>
                    </div>
                  </td>
                    ...
                    <div className="text-xs text-slate-500 space-y-0.5">
                      ...
                      {rpt.leave_deduction > 0 && (
                        <div className="flex justify-between w-40 text-red-500">
                          <span>請假扣款:</span> <span>-${rpt.leave_deduction}</span>
                        </div>
                      )}
                    </div>
                  </td>
                    ...
                    <div className="flex flex-col gap-2 items-center">
                      ...
                      <button 
                        ...
                        className={`w-full py-1.5 px-3 rounded-lg transition text-xs font-bold flex items-center justify-center gap-1 ${
                          expandedRow === idx ? 'bg-slate-800 text-white' : 'text-slate-400 hover:bg-slate-100'
                        }`}
                      >
                        {expandedRow === idx ? (
                          <>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="bg-white p-4 border border-blue-100 rounded-xl shadow-sm">
                          ...
                          <ul className="space-y-2 text-sm text-slate-600">
                            <li className="flex justify-between">
                              <span>本薪 ({rpt.normal_hours}hr):</span>
                              <span className="font-mono font-bold">${rpt.base_pay}</span>
                            </li>
                            <li className="flex justify-between">
                              <span>平日加班 ({rpt.normal_ot_hours}hr):</span>
                              <span className="font-mono">
                                ${calculateTieredOt(rpt.normal_ot_hours, Math.round(rpt.base_pay/240))}
                              </span>
                            </li>
                            <li className="flex justify-between">
                              <span>休息日/國定 ({rpt.rest_work_hours + rpt.holiday_work_hours}hr):</span>
                              <span className="font-mono">${rpt.ot_pay + rpt.holiday_pay}</span>
                            </li>
                            {rpt.period_ot_hours > 0 && (
                              <li className="flex justify-between text-orange-600 font-bold">
                                <span>週期總量超時 ({rpt.period_ot_hours.toFixed(1)}hr):</span>
                                <span>(已計入加班)</span>
                              </li>
                            )}
                            {(rpt.bonus_details || []).map((b:any, i:number) => (
                              <li key={i} className="flex justify-between text-green-700">
                                <span>[津貼] {b.name}:</span>
                        ...
                        <div className="bg-white p-4 border border-red-100 rounded-xl shadow-sm">
                          ...
                          <ul className="space-y-2 text-sm text-slate-600">
                            ...
                            <li className="flex justify-between">
                              <span>固定扣項:</span>
                              <span className="font-mono">${rpt.fixed_deduction_pay}</span>
                            </li>
